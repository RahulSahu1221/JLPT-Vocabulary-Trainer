...
/* ═══════════════════════════════════════════════════════════════════════════
   SERVICE WORKER — JLPT N5–N4 Vocabulary Trainer
...
const CACHE_STATIC   = `jlpt-static-${CACHE_VERSION}`;
const CACHE_LESSONS  = `jlpt-lessons-${CACHE_VERSION}`;
const CACHE_CDN      = `jlpt-cdn-${CACHE_VERSION}`;
...
self.addEventListener("push", event => {
    let payload = { title: "JLPT Trainer 🎌", body: "You have SRS reviews due!" };
    try {
        if (event.data) payload = event.data.json();
    } catch (_) { /* malformed push payload — use default */ }

    event.waitUntil(
        self.registration.showNotification(String(payload.title || "JLPT Trainer"), {
            body:    String(payload.body  || "Time to review!"),
            icon:    "./data/assets/logo.jpg",
            badge:   "./data/assets/logo.jpg",
            vibrate: [200, 100, 200],
